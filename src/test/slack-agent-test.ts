...
const outputDir = path.join(__dirname, 'output', 'slack');

/**
 * Test for Slack A2A agent
 */
async function testSlackAgent() {
  console.log("===== A2A Slack Agent Test =====");
  ...
  const serverUrl = "http://localhost:41243"; // Slack agent URL
  ...
  try {
    ...
    // Test message for Slack
    const message = "Send a message to #test-slack-damien saying 'This is a test message from the A2A Slack agent test'";
    ...
    console.log(`\n🚀 Sending request to Slack Agent...`);
    ...
    console.log(`\n✅ Communication with Slack Agent completed!`);
  } catch (error) {
    console.error(`\n❌ Error communicating with Slack Agent:`, error);
    process.exit(1);
  }
}

// Run the test
testSlackAgent().catch(err => {
